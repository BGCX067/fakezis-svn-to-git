...
#AutoIt3Wrapper_Outfile=FakeZIS_v4.0.exe
...
Dim $szIPADDRESS = IniRead("FakeZIS.ini", "main", "ipoverrule", "")
If $szIPADDRESS = "" Then
...
Dim $fmserverport = IniRead("FakeZIS.ini", "main", "fmserverport", "591")
Dim $fmserverip = IniRead("FakeZIS.ini", "main", "fmserverip", "localhost")
Dim $erreur = ""
Dim $debugfile = @scriptdir & "/debug.txt", $data, $debug



; Create a GUI for messages
;==============================================
Dim $GOOEY = GUICreate("FakeZIS v4.0 - for extreme HL7 developers with FileMaker link", 500, 670)
		...
		While 1
			...
			If $recv <> "" Then
				...
				If $ConnectedSocket > -1 Then
					...
					If StringInStr($recv, "help", "", 1) Then
						$returnFile = "help.txt"
					ElseIf StringInStr($recv, "QRY^Q01", "", 1) Then

						$UrlToGet = "http://mhp:mhp@" & $fmserverip & ":" & $fmserverport & "/FMPro?-db=Pat.fp5&-format=QRYQ01_24.htm&-lay=SQL&PATidnum===" & $PatID & "&-max=1&-find"
						ConsoleWrite($UrlToGet)
						$erreur = InetGet($UrlToGet, @ScriptDir & "\temp.txt", 1)
						$returnText = FileRead(@ScriptDir & "\temp.txt")
						_log ( "inetget: " , $erreur)
						_log ( "returntext: " , $returnText )

						If StringInStr($returnText, "No Records Found") Then
							$returnText = "MSA|AA|2|Geen patient(en) gevonden|"
							$returnText &= "QRD|200706121420|R|I|QRY-1|||99^RD|1108304|RES"
							$returnText &= "QRF|CCS|||||COL|FIN|ALL|^^^198001010000"
							$returnText &= "OBR|1||^||||||||||||||||||||||||"
							$returnText &= ""
						EndIf
						$returnFile = "FileMaker Query";
					Else
						$returnFile = $querytype & "_" & $HL7version & ".txt"
					EndIf

					If $returnFile = "FileMaker Query" Then
						;; filemaker verzorgt het antwoord
						$reply = $returnText
						GUICtrlSetData($edit, "Returned text:" & " >> " & $returnText& @CRLF & GUICtrlRead($edit))

					Else
						;; de text file verzorgt het antwoord.
						...
						Else
							$reply = "No return file specified for query. File required: " & $returnFile
						EndIf
					...
					GUICtrlSetData($edit, _
							$szIP_Accepted & " << " & $ret & " characters returned (incl stop Chars) at " & @YEAR & @MON & @MDAY & @HOUR & @MIN & @CRLF & GUICtrlRead($edit))
...
Func _log($what, $value = "")
	If $debug = 1 Then
		$data = "Debug info: " & @YEAR & @MON & @MDAY & "@" & @HOUR & @MIN & " - " & $what & " (Err:" & @error & " Value: " & $value & ")" & @CRLF
		ConsoleWrite($data)
